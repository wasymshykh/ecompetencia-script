<?php
    ...
    function isAmbassador($id) {
        global $db;
        $q = "SELECT * FROM `ambassadors` WHERE `ambassador_ID`=:id AND `ambassador_status`='E'";
        $s = $db->prepare($q);
        $s->execute(['id'=>$id]);
        if($s->rowCount() === 1){
            return true;
    ...
    function isValidLogin($id, $pass) {
        global $db;
        $q = "SELECT * FROM `ambassadors` WHERE `ambassador_ID`=:id AND `ambassador_password`=:password";
        $s = $db->prepare($q);
    ...
    function getUnconfirmedParticipants($am_id)
    {
        global $db;
        $q = "SELECT * FROM `participants` p INNER JOIN `users` u ON p.user_ID = u.user_ID INNER JOIN `transactions` t ON p.participant_ID = t.participant_ID INNER JOIN `competitions` c ON p.competition_ID = c.competition_ID WHERE t.transaction_status='U' AND u.ambassador_ID=:am_id";
        $s = $db->prepare($q);
        $s->execute(['am_id'=>$am_id]);
        return $s->fetchAll();
    }

    function getUnconfirmedById($part_id, $am_id)
    {
        global $db;
        $q = "SELECT * FROM `participants` p INNER JOIN `users` u ON p.user_ID = u.user_ID INNER JOIN `transactions` t ON p.participant_ID = t.participant_ID INNER JOIN `competitions` c ON p.competition_ID = c.competition_ID WHERE t.transaction_status='U' AND p.participant_ID=:partid AND u.ambassador_ID=:am_id";
        $s = $db->prepare($q);
        $s->execute(['partid'=>$part_id, 'am_id'=>$am_id]);
        return $s->fetch();
    }

    function getUnconfirmedInstituteParticipants($ins_id)
    {
        global $db;
        $q = "SELECT * FROM `participants` p INNER JOIN `users` u ON p.user_ID = u.user_ID INNER JOIN `transactions` t ON p.participant_ID = t.participant_ID INNER JOIN `competitions` c ON p.competition_ID = c.competition_ID WHERE t.transaction_status='U' AND u.institute_ID=:ins_id";
        $s = $db->prepare($q);
        $s->execute(['ins_id'=>$ins_id]);
        return $s->fetchAll();
    }

    function getUnconfirmedInsPartById($part_id, $ins_id)
    {
        global $db;
        $q = "SELECT * FROM `participants` p INNER JOIN `users` u ON p.user_ID = u.user_ID INNER JOIN `transactions` t ON p.participant_ID = t.participant_ID INNER JOIN `competitions` c ON p.competition_ID = c.competition_ID WHERE t.transaction_status='U' AND p.participant_ID=:partid AND u.institute_ID=:ins_id";
        $s = $db->prepare($q);
        $s->execute(['partid'=>$part_id, 'ins_id'=>$ins_id]);
        return $s->fetch();
    }
    ...
    function getConfirmedParticipants($am_id)
    {
        global $db;
        $q = "SELECT * FROM `participants` p INNER JOIN `users` u ON p.user_ID = u.user_ID INNER JOIN `transactions` t ON p.participant_ID = t.participant_ID INNER JOIN `transaction_details` td ON t.transaction_ID = td.transaction_ID INNER JOIN `competitions` c ON p.competition_ID = c.competition_ID WHERE t.transaction_status='P' AND td.paid_to='A' AND td.details_receiver_ID=:am_id";
        $s = $db->prepare($q);
        $s->execute(['am_id'=>$am_id]);
        return $s->fetchAll();
    }
